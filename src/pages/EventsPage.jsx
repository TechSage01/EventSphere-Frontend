  ...
  return (
    <div style={{ ...styles.shell, ...(responsive?.shell || {}) }}>
      ...
      <header style={{ ...styles.topbar, ...(responsive?.topbar || {}) }}>
        <nav style={{ ...styles.topbarLeft, ...(responsive?.topbarLeft || {}) }}>
          ...
          <a href="/events" style={styles.navItem} title="Show Events" data-active="true">
            <span style={styles.navIcon}>▦</span> {viewportWidth > 580 && 'Events'}
          </a>
          <a href="/calendars" style={{ ...styles.navItem, color: '#8a8a8a' }} title="Show Calendar">
            <span style={styles.navIcon}>📅</span> {viewportWidth > 580 && 'Calendars'}
          </a>
          <a href="/discover" style={{ ...styles.navItem, color: '#8a8a8a' }} title="Show Discover">
            <span style={styles.navIcon}>◎</span> {viewportWidth > 580 && 'Discover'}
          </a>
        </nav>

        <div style={{ ...styles.topbarRight, ...(responsive?.topbarRight || {}) }}>
          {viewportWidth > 400 && (
            <span style={styles.timeChip}>{useCurrentTime()}</span>
          )}

          <button type="button" style={{ ...styles.createBtn, ...(responsive?.createBtn || {}) }} onClick={() => navigate('/events/new')}>
      ...
      <main style={{ ...styles.main, ...(responsive?.main || {}) }}>
        ...
        <div style={{ ...styles.pageHead, ...(responsive?.pageHead || {}) }}>
          ...
          <div style={{ ...styles.toggle, ...(responsive?.toggle || {}) }} role="tablist">
            <button
              role="tab"
        ...
        {/* ── content ── */}
        {loading ? (
          <EmptyState tab={tab} navigate={navigate} message="Loading events..." responsive={responsive} />
        ) : error ? (
          <EmptyState tab={tab} navigate={navigate} message={error} responsive={responsive} />
        ) : visibleEvents.length === 0 ? (
          <EmptyState tab={tab} navigate={navigate} responsive={responsive} />
        ) : (
          <div style={{ ...styles.evGrid, ...(responsive?.evGrid || {}) }}>
            {visibleEvents.map(ev => <EventCard key={ev.id} ev={ev} onOpen={() => navigate(`/events/${ev.id}`)} responsive={responsive} />)}
          </div>
        )}
      </main>
    </div>
...
function EmptyState({ tab, navigate, message = '', responsive = null }) {
  return (
    <div style={{ ...styles.emptyWrap, ...(responsive?.emptyWrap || {}) }}>
      <div style={{ ...styles.emptyIconWrap, ...(responsive?.emptyIconWrap || {}) }}>
      ...
      <button type="button" style={{ ...styles.createBtnLarge, ...(responsive?.createBtnLarge || {}) }} onClick={() => navigate('/events/new')}>
...
/* ─── event card ─── */
function EventCard({ ev, onOpen, responsive = null }) {
  ...
  return (
    <div style={{ ...styles.card, ...(responsive?.card || {}) }} onClick={onOpen} role="button" tabIndex={0}>
      ...
      <div style={{ ...styles.cardBody, ...(responsive?.cardBody || {}) }}>
        ...
        <p style={{ ...styles.cardTitle, ...(responsive?.cardTitle || {}) }}>{ev.title}</p>
        <p style={{ ...styles.cardLoc, ...(responsive?.cardLoc || {}) }}>{ev.location ? `📍 ${ev.location}` : '📍 Location not set'}</p>
        <p style={{ ...styles.cardLoc, ...(responsive?.cardLoc || {}) }}>{timeLabel}</p>
        <div style={{ ...styles.cardFoot, ...(responsive?.cardFoot || {}) }}>
          <span style={{ ...styles.cardGoing, ...(responsive?.cardGoing || {}) }}>{ev.hostName || ev.hostEmail || 'Creator'} </span>
          <span style={{ ...styles.cardTag, ...(responsive?.cardTag || {}), background: ev.isPublic ? '#1f3d2c' : '#3a2431', color: ev.isPublic ? '#86efac' : '#f9a8d4' }}>{statusLabel}</span>
        </div>
      </div>
    </div>
  )
...
const styles = {
  ...
  starLogo: { fontSize: 18, color: '#a78bfa', marginRight: 16, cursor: 'pointer' },
  navItem: {
    display: 'flex', alignItems: 'center', gap: 6,
    fontSize: 13.5, fontWeight: 500, color: '#e8e8ec',
    textDecoration: 'none', padding: '5px 11px', borderRadius: 8,
    transition: 'background .12s', cursor: 'pointer',
  },
  navIcon: { fontSize: 13, opacity: .75 },
  topbarRight: { display: 'flex', alignItems: 'center', gap: 8 },
  timeChip: { fontSize: 12, color: '#6b6b76', fontVariantNumeric: 'tabular-nums' },
  createBtn: {
    fontSize: 12.5, fontWeight: 600, color: '#e8e8ec',
    background: 'rgba(255,255,255,0.10)', border: '1px solid rgba(255,255,255,0.12)',
    padding: '6px 14px', borderRadius: 999, cursor: 'pointer', transition: 'background .12s',
  },
  logoutBtn: {
    fontSize: 12.5, fontWeight: 600, color: '#f0f0f4',
    background: 'rgba(248,113,113,0.13)', border: '1px solid rgba(248,113,113,0.3)',
    padding: '6px 12px', borderRadius: 999, cursor: 'pointer',
    fontFamily: "'DM Sans', system-ui, sans-serif",
  },
  iconBtn: {
    background: 'none', border: 'none', color: '#6b6b76', cursor: 'pointer',
    padding: 6, borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center',
  },
  avatar: {
    width: 30, height: 30, borderRadius: '50%', background: '#f5c842',
    display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer',
  },
  main: { maxWidth: 900, margin: '0 auto', padding: '40px 28px 80px' },
  pageHead: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 32 },
  pageTitle: { fontSize: 26, fontWeight: 700, letterSpacing: '-.5px', color: '#f0f0f4' },
  toggle: { display: 'flex', background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: 10, padding: 3, gap: 2 },
  toggleBtn: {
    fontSize: 13, fontWeight: 500, color: '#6b6b76', background: 'none', border: 'none',
    padding: '6px 16px', borderRadius: 7, cursor: 'pointer', fontFamily: "'DM Sans', system-ui, sans-serif",
  },
  toggleBtnActive: { background: 'rgba(255,255,255,0.10)', color: '#f0f0f4' },
  emptyWrap: { display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', paddingTop: 80, paddingBottom: 60, textAlign: 'center' },
  emptyIconWrap: { marginBottom: 28 },
  emptyIcon: { opacity: .85 },
  emptyTitle: { fontSize: 22, fontWeight: 600, color: '#8a8a96', marginBottom: 10 },
  emptyBody: { fontSize: 14, color: '#55555e', marginBottom: 32, lineHeight: 1.6 },
  createBtnLarge: { display: 'inline-flex', alignItems: 'center', textDecoration: 'none', fontSize: 14, fontWeight: 600, color: '#e8e8ec', background: 'rgba(255,255,255,0.10)', border: '1px solid rgba(255,255,255,0.13)', padding: '11px 24px', borderRadius: 999, cursor: 'pointer' },
  evGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 16 },
  card: { background: '#1c1c24', border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, overflow: 'hidden', cursor: 'pointer' },
  cardCover: { height: 110, display: 'flex', alignItems: 'center', justifyContent: 'center' },
